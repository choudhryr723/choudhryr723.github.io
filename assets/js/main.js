/* Rakesh Kumar Choudhary — portfolio
...
(() => {
  ...
  const COMPANY_ORDER = ['Rayz Infotech', 'Digitalries', 'Cortech', 'Oppenheimer', 'Fever'];
      ...
      <div class="modal-hero">
        <div class="modal-head">
          ...
          <a class="m-play" href="${esc(app.url)}" target="_blank" rel="noopener">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 17 17 7M9 7h8v8"/></svg>
            ${app.url && app.url.includes('play.google') ? 'Get it on Google Play' : 'View project'}
          </a>
        </div>
      </div>
  ...
  const loadApps = async () => {
    ...
    const playCount = (data.workApps || []).filter(a => (a.url || '').includes('play.google')).length;
    statsFinal = { apps: `${allApps.length}+`, installs: `${playCount || 5}+` };
    maybeRunStats();
  ...
  loadApps().catch((err) => {
    console.error(err);
    $('#myApps').innerHTML = '<p style="color:var(--muted);font-family:var(--font-jakarta);font-size:0.85rem">Could not load app data — see them all on <a href="https://play.google.com/store/apps/details?id=com.rayz.bcpl">Google Play</a>.</p>';
  });
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch('https://formsubmit.co/ajax/choudharyr723@gmail.com', {
          method: 'POST',
        ...
        formMsg.innerHTML = 'Could not send right now — email me directly at <a href="mailto:choudharyr723@gmail.com">choudharyr723@gmail.com</a>.';
